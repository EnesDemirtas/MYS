...
! function(a, b) {
    ...
    function D(a, b, c) {
        if (b.enablePagination) {
            var d = a.find(".actions btn[type$='submit']").parent(),
                e = a.find(".actions a[href$='#Sonraki']").parent();
            if (!b.forceMoveForward) {
                var f = a.find(".actions a[href$='#Önceki']").parent();
    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
                ...
                f = "";
            b.forceMoveForward || (f += e.format("Önceki", b.labels.previous)), f += e.format("Sonraki", b.labels.next), b.enableCancelButton && (f += e.format("İptal et", b.labels.cancel)), a.append(d.format(b.actionContainerTag, b.clearFixCssClass, b.labels.pagination, f)), D(a, b, c), A(a, b, c)
        }
    }
